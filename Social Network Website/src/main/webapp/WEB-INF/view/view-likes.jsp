...
<body>
	...
	<h1>This post received <c:out value="${count}"></c:out> reactions</h1>
	<br>
	<c:if test="${employeeLike.size() > 0}">
		<h2>The following people reacted &#128522;</h2>
		<c:forEach var="employee" items="${employeeLike}">
			<c:url var="reactor" value="/employee/searchUserProfile">
			<c:param name="searchUserId" value="${employee.getId()}" />
		</c:url>
		<a href="${reactor}" style="font-size: 15px;"><c:out value="${employee.getFirstName()} ${employee.getLastName()}"/></a>
		<br>
		</c:forEach>
	</c:if>
	<br><br><br>
	<c:if test="${employeeLaugh.size() > 0}">
		<h2>The following people reacted &#128514;</h2>
		<c:forEach var="employee" items="${employeeLaugh}">
			<c:url var="reactor" value="/employee/searchUserProfile">
			<c:param name="searchUserId" value="${employee.getId()}" />
		</c:url>
		<a href="${reactor}" style="font-size: 15px;"><c:out value="${employee.getFirstName()} ${employee.getLastName()}"/></a>
	...
	<br><br><br>
	<c:if test="${employeeCried.size() > 0}">
		<h2>The following people reacted &#128557;</h2>
		<c:forEach var="employee" items="${employeeCried}">
			<c:url var="reactor" value="/employee/searchUserProfile">
			<c:param name="searchUserId" value="${employee.getId()}" />
		</c:url>
		<a href="${reactor}" style="font-size: 15px;"><c:out value="${employee.getFirstName()} ${employee.getLastName()}"/></a>
	...
	<br><br><br>
	<c:if test="${employeeAngry.size() > 0}">
		<h2>The following people reacted &#128544;</h2>
		<c:forEach var="employee" items="${employeeAngry}">
			<c:url var="reactor" value="/employee/searchUserProfile">
			<c:param name="searchUserId" value="${employee.getId()}" />
		</c:url>
		<a href="${reactor}" style="font-size: 15px;"><c:out value="${employee.getFirstName()} ${employee.getLastName()}"/></a>
